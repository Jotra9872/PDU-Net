...
# data_path: 新服务器 /media/jiangaiwen/DATA_Wu/Dehaze  旧服务器 /media/jiangaiwen/DATA_OLD/Wu/Dehaze  自己电脑 G:/Dehaze
...
parser.add_argument('--data_path', default='D:/Dehaze', type=str, help='path to dataset')  # 这个是dataset的路径(大路径) 和下面的train_set合起来就是训练集的路径
...
# ---------训练不同数据集需要修改的:
# O-Haze I-Haze Dense-Haze  NH-Haze  NH-Haze2  Haze4K  RSHaze  RESIDE/ITS/  RESIDE/SOTS/SOTS-IN/
parser.add_argument('--train_path', default='NH-Haze2/Train', type=str, help='train dataset name')  # 训练集目标路径
parser.add_argument('--val_path', default='NH-Haze2/Test', type=str, help='valid dataset name')  # 测试集目标路径
parser.add_argument('--train_data', default='NH-Haze2', type=str, help='experiment setting')  # 保存日志的时候用到的,到底是用到的哪个数据集 RESIDE_ITS  RESIDE_OTS
...
parser.add_argument('--Last_train_model_path', default='saved_models_stage1/NH-Haze2/DehazeNet1_10/DehazeNet1_10_2228_21.5144.pth', help="path to continue training")
...
setting = {'batch_size': 8,
           ...
           'only_h_flip': False,
           'optimizer': "adamw",
           'lr': 2e-4,  # 64--8e-4  128--16e-4  32--4e-4 16--2e-4  8--1e-4
           'eval_freq': 1,
           'weight_decay': 0.01,
           'epochs': 6000,
           'warmup_epochs': 50,
...
if __name__ == '__main__':
    ...
    optimizer = torch.optim.AdamW(params=filter(lambda x: x.requires_grad, network.parameters()), lr=setting['lr'], weight_decay=setting['weight_decay'])
    ...
    train_loader = DataLoader(train_dataset,
                              ...
                              pin_memory=True,
                              drop_last=True)
    val_dataset = PairLoader(os.path.join(args.data_path, args.val_path), 'test')
    ...
    for epoch in range(cur_epoch, setting['epochs'] + 1):
        ...
        if epoch % setting['eval_freq'] == 0:
            ...
            if avg_psnr > best_psnr:
                best_psnr = avg_psnr
                state = {'state_dict': network.state_dict(), 'best_psnr': best_psnr}  # state变量存放训练过程中需要学习的权重和偏执系数还要当前的best_psnr
